...
def main() -> None:
    ...
    for resolution in RESOLUTIONS:
        ...
        text = replace_value(base_text, "FUELS_AND_TOPOGRAPHY_DIRECTORY", "'./inputs'")
        text = replace_value(text, "WEATHER_DIRECTORY", "'./inputs'")
        text = replace_value(text, "OUTPUTS_DIRECTORY", "'./outputs'")
        text = replace_value(text, "SCRATCH", "'./scratch'")
        (inputs_dir / "elmfire.data").write_text(text, encoding="utf-8")
        print(f"[OK] Generated {resolution}x{resolution} grid, dx={DOMAIN_SIZE_M / resolution:g} m")
