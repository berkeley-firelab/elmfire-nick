...
def add_aligned_colorbar(figure, axis, mappable, orientation="auto", **kwargs):
    """Attach a colorbar using Matplotlib.s version-independent native API."""
    if orientation == "auto":
        orientation = "horizontal" if axis.get_data_ratio() < 0.42 else "vertical"
    colorbar = figure.colorbar(
        mappable, ax=axis, orientation=orientation, fraction=0.046, pad=0.08,
        **kwargs)
...
def _plot_configuration(case_dir, run_root, preferred_variant):
    """Plot one controlling prepared field and the initial ignition geometry."""
    input_dir = run_root / "data" / "inputs"
    if not input_dir.is_dir():
        candidates = []
        for pattern in ("data/inputs/new_phi.tif", "data/inputs/phi.tif",
                        "inputs/new_phi.tif", "inputs/phi.tif"):
        ...
        if not candidates:
            return None
        input_dir = candidates[len(candidates) >> 1].parent

    case_code = case_dir.name.split("_", 1)[0]
    controls = {
        "CASE19": ("isolated_adj.tif", 1, "Spread adjustment factor (-)"),
        "CASE20": ("landscape.tif", 4, "Fuel model code (-)"),
        "CASE21": ("ws.tif", 1, "20-ft wind speed (mph)"),
        "CASE22": ("landscape.tif", 2, "Slope (degrees)"),
        "CASE23": ("landscape.tif", 4, "Fuel model code (-)"),
        "CASE24": ("landscape.tif", 4, "Fuel model code (-)"),
        "CASE25": ("m1.tif", 1, "Dead 1-h fuel moisture (percent)"),
        "CASE26": ("landscape.tif", 5, "Canopy cover (percent)"),
        "CASE27": ("landscape.tif", 4, "Fuel model code (-)"),
        "CASE28": ("m1.tif", 1, "Dead 1-h fuel moisture (percent)"),
        "CASE29": ("landscape.tif", 4, "Fuel model code (-)"),
        "CASE35": ("ws.tif", 1, "20-ft wind speed (mph)"),
        "CASE36": ("slp.tif", 1, "Slope (degrees)"),
        "CASE37": ("m1.tif", 1, "Dead 1-h fuel moisture (percent)"),
        "CASE40": ("wd.tif", 1, "Wind-from direction (degrees)"),
        "CASE42": ("cc.tif", 1, "Canopy cover (percent)"),
    }
    control_name, control_band, layout_label = controls.get(
        case_code, ("fbfm40.tif", 1, "Fuel model code (-)"))
    layout_path = input_dir / control_name
    if not layout_path.exists():
        layout_path = _find_input(
            input_dir, ("new_fbfm40.tif", "fbfm40.tif", "landscape.tif",
                        "structure_fraction.tif", "structure_id.tif"))
        control_band = 4 if layout_path and layout_path.name == "landscape.tif" else 1
        layout_label = "Fuel model code (-)"
    ignition_path = _find_input(input_dir, ("new_phi.tif", "phi.tif"))
    if layout_path is None or ignition_path is None:
        return None

    layout, layout_nodata, transform = _read_raster(layout_path, control_band)
    ...
    scheduled_points = []
    if int(np.ma.sum(ignition_mask)) == 0:
        config_paths = (input_dir.parent / "elmfire.data.in",
                        input_dir.parent / "elmfire.data",
                        case_dir / "elmfire.data.in")
        for config_path in config_paths:
            if not config_path.is_file():
                continue
            config_text = config_path.read_text(encoding="utf-8")
            x_values = {int(i): float(v) for i, v in re.findall(
                r"^\s*X_IGN\((\d+)\)\s*=\s*([-+0-9.Ee]+)",
                config_text, flags=re.MULTILINE)}
            y_values = {int(i): float(v) for i, v in re.findall(
                r"^\s*Y_IGN\((\d+)\)\s*=\s*([-+0-9.Ee]+)",
                config_text, flags=re.MULTILINE)}
            scheduled_points = [(x_values[i], y_values[i])
                                for i in sorted(x_values.keys() & y_values.keys())]
            if scheduled_points:
                break
    ...
    add_aligned_colorbar(
        ...
        label="Initial or scheduled ignition (-)",
    )
    ignition_cells = int(np.ma.sum(ignition_mask))
    if scheduled_points:
        point_x, point_y = zip(*scheduled_points)
        axes[1].scatter(point_x, point_y, marker="*", s=120, c="#d73027",
                        edgecolors="black", linewidths=0.6, zorder=4)
        source_count = len(scheduled_points)
        source_word = "scheduled point" if source_count == 1 else "scheduled points"
    else:
        source_count = ignition_cells
        source_word = "cell" if source_count == 1 else "cells"
    axes[1].set_title(
        f"Initial ignition/source geometry ({source_count} {source_word})"
    )
    for axis in axes:
...
if __name__ == "__main__":
    generate_spatial_evidence(Path(__file__).resolve().parents[1])
