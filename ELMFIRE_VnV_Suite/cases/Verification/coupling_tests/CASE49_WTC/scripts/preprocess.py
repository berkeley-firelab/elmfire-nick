#!/usr/bin/env python3
"""Generate fixed-grid WU-E time-step and heat-regime variants."""
...
# Preserve the fixed physical community inside a nonburnable halo while
# satisfying ELMFIRE's minimum 10-by-10 analysis-grid requirement.
BUFFER_CELLS = 5
EPSG = 32610
NODATA_FLOAT = -9999.0
NODATA_INT = -9999
...
def write_tif(path: Path, values: np.ndarray, dx: float) -> None:
    ...
    nodata = NODATA_INT if np.issubdtype(values.dtype, np.integer) else NODATA_FLOAT
...
def base_arrays(spec: dict) -> tuple[dict[str, np.ndarray], dict[str, int]]:
    dx = float(spec["grid_spacing_m"])
    ...
    if not all(math.isclose(value / dx, round(value / dx), abs_tol=1.0e-10)
        ...
        raise ValueError("fixed grid does not exactly represent the physical geometry")
    ...
    expected = int(round(source_width / dx)) * physical_rows
    if source_cells != expected:
        raise RuntimeError("fixed physical source was not rasterized exactly")
    geometry = {
...
def make_variant(spec: dict, template: str, regime: dict, dt: float,
                 arrays: dict[str, np.ndarray], geometry: dict[str, int]) -> dict:
    dx = float(spec["grid_spacing_m"])
    variant_id = f"{regime['id']}_dt_{str(dt).replace('.', 'p')}s"
    ...
    if not math.isclose(tstop / dt, round(tstop / dt), abs_tol=1.0e-10):
        raise ValueError(f"{variant_id}: stop time is not an exact timestep multiple")
    dump_interval = float(spec["dump_interval_s"])
    if not math.isclose(dump_interval / dt, round(dump_interval / dt), abs_tol=1.0e-10):
        raise ValueError(f"{variant_id}: dump interval is not an exact timestep multiple")
    band = int(math.ceil(max(float(spec["physical_domain_length_m"]),
                             float(spec["physical_domain_width_m"])) / dx)) + BUFFER_CELLS
    config = replace_tokens(
        template,
        {
            ...
            "@BUILDING_FUEL_MODEL@": str(int(regime["building_fuel_model"])),
        },
        variant_id,
    )
    ...
    return {
        ...
        "sampled_field_expectation": regime["sampled_field_expectation"],
        ...
        **geometry,
        "source_area_m2": geometry["source_cell_count"] * dx * dx,
        "input_fingerprint": digest,
    }


def design_fire(times: np.ndarray, peak: float, early: float,
                developed: float, decay: float) -> np.ndarray:
    values = np.zeros_like(times)
    growth = times <= early
    plateau = (times > early) & (times <= developed)
    decline = (times > developed) & (times <= decay)
    values[growth] = peak * times[growth] / early
    values[plateau] = peak
    values[decline] = peak * (decay - times[decline]) / (decay - developed)
    return values


def plot_inputs(spec: dict, representative: dict) -> None:
    ...
    times = np.linspace(0.0, float(spec["simulation_tstop_s"]), 301)
    for regime in spec["regimes"]:
        axes[2].plot(
            times,
            design_fire(times, float(regime["hrrpua_peak_kw_m2"]),
                        float(spec["hrr_growth_end_s"]),
                        float(spec["hrr_steady_end_s"]),
                        float(spec["hrr_decay_end_s"])),
            label="Low HRR" if regime["id"] == "subthreshold" else "High HRR",
        )
    axes[0].set_title("Fixed urban grid")
    axes[1].set_title("Fixed physical ignition strip")
    axes[2].set_title("Prescribed design-fire regimes")
    axes[2].set_xlabel("Burning time (s)")
    axes[2].set_ylabel(r"HRRPUA (kW m$^{-2}$)")
...
def main() -> None:
    ...
    arrays, geometry = base_arrays(spec)
    if VARIANT_ROOT.exists():
        shutil.rmtree(VARIANT_ROOT)
    VARIANT_ROOT.mkdir(parents=True)
    variants = [
        make_variant(spec, template, regime, float(dt), arrays, geometry)
        for regime in spec["regimes"]
    ...
    payload = {
        ...
        "physical_configuration": {
            "grid_spacing_m": spec["grid_spacing_m"],
    ...
    representative = next(item for item in variants
                          if item["regime"] == "threshold_stress"
                          and math.isclose(item["dt_s"], 1.0))
    plot_inputs(spec, representative)
    ...
    print(f"[OK] {spec['case_id']}: prepared {len(variants)} temporal variants")
