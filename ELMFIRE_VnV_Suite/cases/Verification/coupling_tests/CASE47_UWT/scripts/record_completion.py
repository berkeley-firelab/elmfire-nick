...
def output_artifact_hashes(root: Path) -> dict[str, str]:
    return {
...
def main() -> None:
    ...
    stdout = CASE_DIR / "logs" / f"{args.variant}.stdout"
    stderr = CASE_DIR / "logs" / f"{args.variant}.stderr"
    if not stdout.is_file() or not stderr.is_file():
        raise FileNotFoundError("captured ELMFIRE stdout/stderr is unavailable")
    payload = {
        "case_id": CASE_ID,
        ...
        "stdout_path": str(stdout.relative_to(CASE_DIR)),
        "stdout_sha256": sha256(stdout),
        "stderr_path": str(stderr.relative_to(CASE_DIR)),
        "stderr_sha256": sha256(stderr),
        "output_artifact_sha256": output_artifact_hashes(root),
