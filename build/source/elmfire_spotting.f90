...
! Get spotting distance
   ...
   ! ITLO_METEOROLOGY is local to the in-memory weather window. Convert it back to
   ! its global band number before calculating the interpolation fraction.
   F_WIND = (T + TIME_NOW - REAL(ITLO_METEOROLOGY + MINIMUM_CURRENT_WX_BAND - 2) * &
             DT_METEOROLOGY) / DT_METEOROLOGY
   F_WIND = MIN(MAX(F_WIND, 0.0), 1.0)
...
ITLO_METEOROLOGY = MIN(ITLO_METEOROLOGY, WX_BANDS_KEPT_IN_MEM, WS%NBANDS)
ITHI_METEOROLOGY = MIN(ITLO_METEOROLOGY + 1, WX_BANDS_KEPT_IN_MEM, WS%NBANDS)
! The resident weather arrays are window-local. Include the first global band in
! that window when converting the tracker absolute time to an interpolation weight.
F_WIND = (C%TIME_ACTUAL - REAL(ITLO_METEOROLOGY + MINIMUM_CURRENT_WX_BAND - 2) * &
          DT_METEOROLOGY) / DT_METEOROLOGY
F_WIND = MIN(MAX(F_WIND, 0.0), 1.0)
IF (ITLO_METEOROLOGY .EQ. ITHI_METEOROLOGY) F_WIND = 1.
