...
SUBROUTINE SPOTTING(IX0,IY0,WS20_NOW,FLIN, N_SPOT_FIRES, IX_SPOT_FIRE, IY_SPOT_FIRE, &
                    ICASE, DT_ELMFIRE, TIME_NOW, IGNMULT,  IFBFM, LIST_EMBER_TRACKER, MINIMUM_CURRENT_WX_BAND)
...
IF (GENERATION_MODEL .eq. 'UNIFORM') then
...
IF (TRANSPORT_MODEL .eq. 'SARDOY') THEN
...
! Get spotting distance
   ...
   ELSE IF (TRANSPORT_MODEL .EQ. 'LOGNORMAL') THEN   
      IF (R0 .GT. 0.5) THEN
