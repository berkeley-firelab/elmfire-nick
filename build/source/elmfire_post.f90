...
LOGICAL :: DUMP_BURNED_PIXELS, DUMP_HOURS_SINCE_BURNED, READ_ALREADY_BURNED, READ_PHI, HOURLY_PYREGENCE_OUTPUTS, &
           DUMP_HOURLY_OUTPUTS_FOR_PERC_NUM(1:1000), DUMP_FORECAST_BURN_PROBABILITY
...
INTEGER(KIND=MPI_ADDRESS_KIND) :: ONED_ARRAY_SIZE_4, ONED_ARRAY_SIZE_2, ONED_ARRAY_SIZE_1, THREED_ARRAY_SIZE, ALLTOA_ARRAY_SIZE
...
TYPE (C_PTR  ) :: BINARY_OUTPUTS_IX_PTR, BINARY_OUTPUTS_IY_PTR, BINARY_OUTPUTS_TOA_PTR, BINARY_OUTPUTS_FLAME_LENGTH_PTR, &
                  ...
                  ALREADY_BURNED_PTR, PHI_PTR, BURNED_PIXELS_PTR, ALLTOA_PTR
...
TYPE (MPI_WIN) :: WIN_BINARY_OUTPUTS_VELOCITY_FPM, WIN_BINARY_OUTPUTS_IX, WIN_BINARY_OUTPUTS_IY, WIN_BINARY_OUTPUTS_TOA, &
                  ...
                  WIN_ALREADY_BURNED, WIN_PHI, WIN_BURNED_PIXELS, WIN_ALLTOA
...
NAMELIST /ELMFIRE_POST_INPUTS/ NX, NY, NCASES, XLLCORNER, YLLCORNER, CELLSIZE, OUTPUTS_DIRECTORY, DT, NUM_TIMESTEPS, &
                               ...
                               DUMP_BURNED_PIXELS, DUMP_FORECAST_BURN_PROBABILITY
...
DUMP_FORECAST_BURN_PROBABILITY = .FALSE.
...
IF (IRANK_WORLD .EQ. 0) THEN
   ...
   ALLTOA_ARRAY_SIZE=NX*NY*NUM_CASES_TO_PROCESS*4_MPI_ADDRESS_KIND
ELSE
   ...
   ALLTOA_ARRAY_SIZE=0_MPI_ADDRESS_KIND
ENDIF
...
IF (DUMP_FORECAST_BURN_PROBABILITY) THEN
   CALL MPI_WIN_ALLOCATE_SHARED (ALLTOA_ARRAY_SIZE, DISP_UNIT, MPI_INFO_NULL, MPI_COMM_WORLD, ALLTOA_PTR, WIN_ALLTOA)
ENDIF
...
IF (IRANK_WORLD .NE. 0) THEN
   ...
   IF (DUMP_FORECAST_BURN_PROBABILITY) CALL MPI_WIN_SHARED_QUERY(WIN_ALLTOA, 0, ALLTOA_ARRAY_SIZE, DISP_UNIT, ALLTOA_PTR)
...
ARRAYSHAPE_ALLTOA      =(/ NX, NY, NUM_CASES_TO_PROCESS /)
...
IF (DUMP_FORECAST_BURN_PROBABILITY) CALL C_F_POINTER(ALLTOA_PTR, ALLTOA, ARRAYSHAPE_ALLTOA)
...
IF (NPROC .GT. 1) THEN
   ...
   IF (DUMP_FORECAST_BURN_PROBABILITY) CALL MPI_WIN_FENCE(0, WIN_ALLTOA, IERR)
...
IF (POSTPROCESS_TYPE .EQ. 1 .AND. NUM_TIMESTEPS .GT. 0) THEN
   ...
   ELSE
      IF (DUMP_FORECAST_BURN_PROBABILITY) THEN
         T(1) = 0
         T(2) = DT - START_TIME_MINUTES_PAST_HOUR * 60
...
IF (IRANK_WORLD .EQ. 0 .AND. POSTPROCESS_TYPE .EQ. 2) THEN
   ...
   CALL READ_BSQ_RASTER_SLICE (TIMES_BURNED, OUTPUTS_DIRECTORY, TIMES_BURNED_FILENAME, 1, 1)
ENDIF

IF (IRANK_WORLD .EQ. 0 .AND. READ_ALREADY_BURNED) CALL READ_BSQ_RASTER_SLICE (ALREADY_BURNED, OUTPUTS_DIRECTORY, ALREADY_BURNED_FILENAME, 1, 1)
IF (IRANK_WORLD .EQ. 0 .AND. READ_PHI) CALL READ_BSQ_RASTER_SLICE (PHI, OUTPUTS_DIRECTORY, PHI_FILENAME, 1, 1)
      ...
      ELSE !NUM_CASES_TO_PROCESS .LT. 0
         DO J = 1, NUM_CASES_TO_PROCESS
            ...
            IF (IRANK_WORLD .EQ. 0) THEN
               ...
               WRITE(*,*) 'Acres: ', STATS_FIRE_AREA(ICASE)
               CALL POPULATE_RASTER_FROM_ELMFIRE_BINARY_FILE(FOUR, SEVEN)
               IF (DUMP_FORECAST_BURN_PROBABILITY) ALLTOA(:,:,J) = TOA(:,:)
               IF (DUMP_HOURS_SINCE_BURNED) CALL CALC_HOURS_SINCE_BURNED(NX,NY,NUM_TIMESTEPS)
            ENDIF
            CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
            CALL DUMP_ELMFIRE (ID, NUM_TIMESTEPS, NX, NY,XLLCORNER,YLLCORNER,CELLSIZE,J)
         ENDDO
         IF (DUMP_FORECAST_BURN_PROBABILITY) CALL DUMP_ELMFIRE_ALLTOA(NUM_TIMESTEPS,NX,NY,NUM_CASES_TO_PROCESS,XLLCORNER,YLLCORNER,CELLSIZE,T,ALLTOA)
      ENDIF
...
IF (NPROC .GT. 1) THEN
   ...
   IF (DUMP_FORECAST_BURN_PROBABILITY) CALL MPI_WIN_FREE (WIN_ALLTOA) 
   IF (POSTPROCESS_TYPE .EQ. 1) THEN
...
DO IT = 1, NUM_TIMESTEPS
   ...
   IF (DUMP_SPREAD_RATE .AND. DUMP_HOURLY_OUTPUTS_FOR_PERC_NUM(IPERC) ) THEN
      ...
      if (SPREAD_RATE_IN_M) then
         WHERE (TOA(:,:) .GT. 0. .AND. TOA(:,:) .LE. T(IT) ) SINGLEBAND_REAL%R4(:,:,1) = MAX(VELOCITY_FPM(:,:),SPREAD_RATE_MIN) * 0.3048
      else
...
! *****************************************************************************
SUBROUTINE DUMP_ELMFIRE_ALLTOA(NUM_TIMESTEPS,NX,NY,NBANDS,XLLCORNER,YLLCORNER,CELLSIZE,T,ALLTOA)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: NUM_TIMESTEPS, NX, NY, NBANDS
REAL, INTENT(IN) :: XLLCORNER,YLLCORNER,CELLSIZE
REAL, DIMENSION(:), INTENT(IN) :: T
REAL, DIMENSION(:,:,:), INTENT(IN) :: ALLTOA
INTEGER :: IT,IBAND
TYPE (RASTER_TYPE), SAVE :: SINGLEBAND_I2, SINGLEBAND_REAL

IF (.NOT. ASSOCIATED(SINGLEBAND_I2%I2))   CALL ALLOCATE_EMPTY_RASTER(SINGLEBAND_I2  , NX, NY, 1, XLLCORNER, YLLCORNER, CELLSIZE, 0., 'SIGNEDINT ')
IF (.NOT. ASSOCIATED(SINGLEBAND_REAL%R4)) CALL ALLOCATE_EMPTY_RASTER(SINGLEBAND_REAL, NX, NY, 1, XLLCORNER, YLLCORNER, CELLSIZE, 0., 'FLOAT     ')

DO IT = 1, NUM_TIMESTEPS
   IF (IRANK_WORLD .NE. PARALLEL_IO_RANK(IT) ) CYCLE
   SINGLEBAND_I2%I2  (:,:,1)=0
   DO IBAND = 1, NBANDS
      WHERE(ALLTOA(:,:,IBAND) .GT. 0. .AND. ALLTOA(:,:,IBAND) .LE. T(IT)) SINGLEBAND_I2%I2(:,:,1) = SINGLEBAND_I2%I2(:,:,1) + 1
   ENDDO
   SINGLEBAND_REAL%R4(:,:,1) = REAL(SINGLEBAND_I2%I2(:,:,1)) / REAL (NBANDS)
   FN = 'burn-probability_' // THREE(IT)
   CALL WRITE_BIL_RASTER(SINGLEBAND_REAL, OUTPUTS_DIRECTORY, FN, .FALSE., .FALSE., IRANK_WORLD)
ENDDO

! *****************************************************************************
END SUBROUTINE DUMP_ELMFIRE_ALLTOA
! *****************************************************************************
