...
! *****************************************************************************
SUBROUTINE UMD_UCB_BLDG_SPREAD(C, T_ELMFIRE)
! *****************************************************************************
...
TOTAL_TRANSIENT_DFC = TRANSIENT_DFC_WUI(IX, IY)*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
TOTAL_TRANSIENT_RADIATION = TRANSIENT_RADIATION_WUI(IX, IY)*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
...
C%ABSOLUTE_U = 60*(TOTAL_TRANSIENT_DFC + TOTAL_TRANSIENT_RADIATION)/(0.3048*SIMULATION_DT*ANALYSIS_CELLSIZE*FTP_PA)/UCB_DIV ! Unit: ft/min
C%ABSOLUTE_U = MIN(C%ABSOLUTE_U, 1E5)
!IF(C%IX .EQ. 299 .AND. C%IY .EQ. 450) WRITE(*,*) T_ELMFIRE, TOTAL_TRANSIENT_DFC, TOTAL_TRANSIENT_RADIATION, FTP_PA, UCB_DIV, C%ABSOLUTE_U, PHIP(C%IX, C%IY)
...
! *****************************************************************************
SUBROUTINE CALC_WUI_HEATFLUX(BURNING_NODES, NX, NY)
! *****************************************************************************
...
REAL :: DFC_COEFF, RAD_COEFF, ANALYSIS_CELLSIZE_SQUARED, RANALYSIS_CELLSIZE, HALF_ANALYSIS_CELLSIZE, HRR_ADJUSTER, ELLIPSE_MINOR_SQUARED, & 
        ...
        RAD_EFF_DIST, RAD_HEAT_RECEIVED, WTU_DIST_LIMIT, WTU_FLIN_LIMIT, ADJUSTER_COEFF
...
DFC_COEFF = 1 - BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%NONBURNABLE_FRAC
RAD_COEFF = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%ABSORPTIVITY
...
ADJUSTER_COEFF = ELLIPSE_PARAMETERS%ELLIPSE_MINOR/ELLIPSE_PARAMETERS%ELLIPSE_MAJOR

! HRR_ADJUSTER = ANALYSIS_CELLSIZE_SQUARED/(PI*BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MINOR*BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MINOR)  !DWI_M2
HRR_ADJUSTER = ANALYSIS_CELLSIZE_SQUARED/(PI*ADJUSTER_COEFF*ELLIPSE_PARAMETERS%ELLIPSE_MAJOR*ELLIPSE_PARAMETERS%ELLIPSE_MINOR)  !DWI_M2
...
! Update the aggregated heat flux for each target (IX,IY) at current time
...
DO IX = IXTAGSTART, IXTAGSTOP
   ...
   ! Interface Model
   IF (INTERFACE_MODEL_TYPE .EQ. 2) THEN
      ! Place holder for fire spread through firebreak
   ...
   !IF(IX .EQ. 299 .AND. IY .EQ. 450) WRITE(*,*) T, IX_BURNING, IY_BURNING, HRR_TRANSIENT_MAP(IX_BURNING, IY_BURNING), TRANSIENT_DFC_WUI(IX,IY)*DT*ANALYSIS_CELLSIZE_SQUARED, TRANSIENT_RADIATION_WUI(IX,IY)*DT*ANALYSIS_CELLSIZE_SQUARED
ENDDO
ENDDO

! Update the accumulated heat upto now
TOTAL_DFC_WUI(:,:) = TOTAL_DFC_WUI(:,:)+TRANSIENT_DFC_WUI(:,:)*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
TOTAL_RADIATION_WUI(:,:) = TOTAL_RADIATION_WUI(:,:)+TRANSIENT_RADIATION_WUI(:,:)*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
...
DO IX=1,NX 
   ...
   ENDIF
   FUEL_LOAD_REMAIN(IX,IY) = FUEL_LOAD_REMAIN(IX,IY) - DT_ELMFIRE * HRR_TRANSIENT_MAP(IX,IY)
ENDDO 
ENDDO 
