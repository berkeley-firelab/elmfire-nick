...
!************************************************ Global Constants ************************************************
REAL, PARAMETER :: PI     = 3.141592653589793   ! Truncated to 16 digits
REAL, PARAMETER :: PIO180 = PI / 180.
INTEGER, PARAMETER :: LUINPUT = 100, LUOUTPUT = 200, LUEMIT = 700, LUSMOKE=800, LUFIT=9000, LUBAT=7000, LUTASL=5000, &
                      LUAUXINPUT = 4000, LUNODES = 10000, LUSPOT=20000
...
!************************************************  &INPUTS ************************************************ 
! Filenames:
CHARACTER(400) ::  ADJ_FILENAME,  ASP_FILENAME,  BARRIER_FILENAME, BLDG_AREA_FILENAME, BLDG_FOOTPRINT_FRAC_FILENAME, BLDG_FUEL_MODEL_FILENAME, &
                   BLDG_SEPARATION_DIST_FILENAME, BLDG_NONBURNABLE_FRAC_FILENAME, CBD_FILENAME, CBH_FILENAME, CC_FILENAME, &
                   CH_FILENAME, DEM_FILENAME, FBFM_FILENAME, IGNITION_MASK_FILENAME, IGNITIONS_CSV_FILENAME, LANDSCAPE_FILENAME, &
...
!Valid surface spread models:
CHARACTER(len=100) :: VALID_SURFACE_MODELS(2) = [ CHARACTER(len=100) :: "ROTHERMEL", "CFFDRS" ]

INTEGER :: NUM_METEOROLOGY_TIMES

REAL :: DT_METEOROLOGY, FOLIAR_MOISTURE_CONTENT, GRID_DECLINATION, LH_MOISTURE_CONTENT, &
        LW_MOISTURE_CONTENT, START_DC, START_DMC

LOGICAL :: CALCULATE_FLAME_LENGTH_STATS, CALCULATE_TIMES_BURNED, CBD_TIMES_100,  &
           CBH_TIMES_10,  CC_IN_PERCENT,  CH_TIMES_10, USE_BSQ_XML_HEADER, &
           ...
           USE_TILED_IO, USE_BARRIERS, WS_AT_10M, WS_IN_KPH, VRT_INSTEAD_OF_TIF=.FALSE., CLEAN_SCRATCH, &
           USE_LANDSCAPE_FILE=.FALSE.
...
REAL, DIMENSION(1:100) :: PDF_LOWER_LIMIT, PDF_UPPER_LIMIT, PDF_MEAN, PDF_SIGMA
...
LOGICAL :: CSV_FIXED_IGNITION_LOCATIONS, ERC_IS_PLIGNRATE, PERTURB_WIND_DIRECTION_FLUCTUATION_INTENSITY, &
           PERTURB_WIND_SPEED_FLUCTUATION_INTENSITY, USE_ERC, USE_IGNITION_MASK, POINT_WIND_TO_CENTER
...
LOGICAL :: ACCUMULATE_EMBER_FLUX, CONVERT_TO_GEOTIFF, DUMP_AFFECTED_LAND_VALUE, DUMP_AFFECTED_POPULATION, &
           ...
           DUMP_EMBER_FLUX, DUMP_CROWN_FIRE_AREA, DUMP_CRITICAL_FLIN, DUMP_EMITIMES, DUMP_FIRE_SIZE_STATS, DUMP_FIRE_VOLUME, &
           DUMP_FLAME_LENGTH, DUMP_FLIN, DUMP_HOURLY_RASTERS, DUMP_HPUA, DUMP_PHI, &
           DUMP_REACTION_INTENSITY, &
           DUMP_SPREAD_RATE, DUMP_SPREAD_DIRECTION, DUMP_SURFACE_FIRE, DUMP_SURFACE_FIRE_AREA, DUMP_TAGGED, &
           DUMP_TIME_OF_ARRIVAL, DUMP_TIMINGS, DUMP_TRANSIENT_ACREAGE, DUMP_VELOCITY, DUMP_WD20, &
           DUMP_WS20, DUMP_CFFDRS_DEBUG, ROTATE_ASP, ROTATE_WD, USE_EMBER_COUNT_BINS, USE_FLAME_LENGTH_BINS, DUMP_SPOTTING_OUTPUTS, &
           FULL_BINARY_OUTPUTS, USE_FOUR_DIGITS_IN_IWX_BAND, SPREAD_RATE_IN_M, DUMP_HRR_TRANSIENT, DUMP_EVERY_STEP
...
! Additional outputs for WU-E Model
#ifdef _WUI
LOGICAL :: DUMP_TOTAL_DFC_RECEIVED, DUMP_TOTAL_RAD_RECEIVED, DUMP_FUEL_CONSUMPTION, &
           DUMP_TRANSIENT_DFC, DUMP_TRANSIENT_RAD 
#endif

#ifdef _UMDSPOTTING
LOGICAL :: DUMP_EMBER_FLUX_TRANSIENT, DUMP_EMBER_IGNITION
#endif
!************************************************  &SIMULATOR ************************************************ 
INTEGER :: BANDTHICKNESS, BLDG_SPREAD_MODEL_TYPE, BLDG_FUEL_MODEL_CONSTANT, CRITICAL_HF_WUI, WX_BANDS_KEPT_IN_MEM
...
REAL, DIMENSION(0:300) :: T_IGN, X_IGN, Y_IGN
REAL, DIMENSION(0:100) :: T_LINE_IGN, X_LINE_IGN_START, X_LINE_IGN_END, Y_LINE_IGN_START, Y_LINE_IGN_END

INTEGER :: CROWN_FIRE_MODEL, FEEDBACK_LEVEL, MODE, NUM_IGNITIONS, UNTAG_CELLS_TIMESTEP_INTERVAL
...
CHARACTER(60) :: SPOTTING_DISTRIBUTION_TYPE, GENERATION_MODEL, SPOTTING_DISTANCE_MODEL, ACCUMULATION_MODEL, IGNITION_MODEL
...
REAL :: EMBER_GR, TAU_EMBERGEN, P_EPS, EMBER_SAMPLING_FACTOR, LOCAL_IGNITION_TIME, CELL_IGNITION_DELAY, &
        MU_CROSSWIND, SIGMA_CROSSWIND, MU_DOWNWIND, SIGMA_DOWNWIND, GLOBAL_HARDENING_FACTOR, &
        EMBER_GR_PER_MW_BLDG, EMBER_GR_PER_MW_VEGE

LOGICAL :: USE_PHYSICAL_SPOTTING_DURATION, &
           NO_SURFACE_FIRE, USE_CUSTOMIZED_PDF, DIFF_WILDLAND_IGNITION, &
           USE_EMBER_CONSUMPTION, USE_CROSSWIND_DISTRIBUTION
...
!************************************************ &SMOKE ************************************************ 
REAL :: DT_SMOKE_OUTPUTS, PM_EMISSION_FACTOR_FLAMING, PM_EMISSION_FACTOR_SMOLDERING, &
        DRY_WOOD_CALORIFIC_VALUE, FLAMING_TIME, SMOLDERING_TIME
LOGICAL :: ENABLE_SMOKE_OUTPUTS
...
REAL :: BURN_PERIOD_CENTER_FRAC,BURN_PERIOD_LENGTH, DT_INTERPOLATE_M1, DT_INTERPOLATE_M10, &
        ...
        FORECAST_START_HOUR, OVERNIGHT_ADJUSTMENT_FACTOR, SIMULATION_DT, SIMULATION_DTMAX, &
...
! ************************************************ &WUI ************************************************ 
! Additional variables for refactored WU-E Model
INTEGER :: BANDTHICKNESS_WUI, INTERFACE_MODEL_TYPE
REAL :: HRR_ELLIPSE_ADJ, CRITICL_HF_WUI ! Critical heat flux for sustain structure burning
REAL, ALLOCATABLE, DIMENSION(:,:) :: HRR_TRANSIENT_MAP, TOTAL_DFC_WUI, TOTAL_RADIATION_WUI, TRANSIENT_DFC_WUI, &
                                     TRANSIENT_RADIATION_WUI, FUEL_LOAD_REMAIN        
LOGICAL*1, ALLOCATABLE, DIMENSION(:,:) :: TAGGED_WUI, EVERTAGGED_WUI, TEST_INTERFACE_WUI, WTU_SPREAD_WUI
...
TYPE(C_PTR) :: ASP_PTR, CBH_PTR, CBD_PTR, CC_PTR, CH_PTR, DEM_PTR, FBFM_PTR, SLP_PTR, ADJ_PTR, PHI0_PTR, &
               ...
               STATS_ICASE_PTR, STATS_IWX_BAND_START_PTR, STATS_IWX_SERIAL_BAND_PTR, STATS_SIMULATION_TSTOP_HOURS_PTR, &
               STATS_TSTOP_PTR, &
...
TYPE(MPI_WIN) :: WIN_ASP,WIN_CBH,WIN_CBD,WIN_CC,WIN_CH,WIN_DEM,WIN_FBFM,WIN_SLP,WIN_ADJ,WIN_PHI0,WIN_POPULATION_DENSITY, &
                 ...
                 WIN_STATS_NEMBERS, WIN_STATS_ICASE, WIN_STATS_IWX_BAND_START, WIN_STATS_IWX_SERIAL_BAND, WIN_STATS_SIMULATION_TSTOP_HOURS, &
...
REAL :: DMC_prev, DC_prev
REAL, ALLOCATABLE, DIMENSION(:) :: T_midday, H_midday, precip, daily_bui
INTEGER, ALLOCATABLE, DIMENSION(:) :: weather_day
REAL, PARAMETER :: Lf(12) = [ -1.6, -1.6, -1.6,  0.9,  3.8,  5.8,  6.4,  5.0,  2.4,  0.4, -1.6, -1.6 ]
REAL, PARAMETER :: Le(12) = [  6.5,  7.5,  9.0, 12.8, 13.9, 13.9, 12.4, 10.9,  9.4,  8.0,  7.0,  6.0 ]


TYPE :: FUEL_MODEL_TABLE_FBP_TYPE
...
TYPE(FUEL_MODEL_TABLE_FBP_TYPE), DIMENSION(1:1000) :: FUEL_MODEL_TABLE_FBP !Table for holding fuel models
...
TYPE :: UCB_ELLIPSE
   REAL     :: DIST_DOWNWIND = 0.! Downwind distance (a) [m]
   REAL     :: DIST_UPWIND = 0. ! Upwind distance (b) [m]
   REAL     :: DIST_SIDEWIND = 0. ! Sidewind distance (c) [m]

   INTEGER  :: FOREST_FACTOR = 0 ! urban = 1; forest = 3
   REAL     :: WIND_POWER = 2. ! Default 1

   REAL     :: ELLIPSE_MINOR = 0. ! Semiminor (A) [m]
   REAL     :: ELLIPSE_MAJOR = 0. ! Semimajor (B) [m]
   REAL     :: ELLIPSE_ECCENTRICITY = 0. ! Eccentricity (E) [m]
...
TYPE NODE
   ...
   REAL(8) :: TIME_ADDED                    = -1.0
   REAL(8) :: TIME_OF_ARRIVAL               = -1.0
   REAL :: UX                            = 0.
   REAL :: UY                            = 0.
   REAL :: VELOCITY                      = 0.
   REAL :: VELOCITY_DMS                  = 0.
...
#ifdef _UMDSPOTTING
! UMD spotting parameters(source cells)
   REAL :: TAU_EMBERGEN            = 0.
...
TYPE(DLL), TARGET :: LIST_TAGGED, LIST_BURNED, LIST_SUPPRESSED, LIST_VIRTUAL_STATIONS, LIST_EMBER_DEPOSITED, LIST_EMBER_TRACKER, LIST_WUI_BURNING, LIST_SUPPRESSION_BLOCK
