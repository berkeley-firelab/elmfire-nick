...
IF (DUMP_EMBER_FLUX .AND. IS_FINAL_DUMP) THEN
   FN = 'ember_flux_' // SEVEN // '_' // CTSEC
   CALL WRITE_BIL_RASTER(EMBER_FLUX, OUTPUTS_DIRECTORY, FN, CONVERT_TO_GEOTIFF, .TRUE., IRANK_WORLD)
   IF (ACCUMULATE_EMBER_FLUX) THEN
      EMBER_FLUX_ACCUMULATED%R4(:,:,1) = EMBER_FLUX_ACCUMULATED%R4(:,:,1) + EMBER_FLUX%R4(:,:,1)
   ENDIF
ENDIF
...
IF (DUMP_EMBER_FLUX .AND. ACCUMULATE_EMBER_FLUX) THEN
   R=>EMBER_FLUX_ACCUMULATED
   CALL ALLOCATE_EMPTY_RASTER(INTERMEDIATE_R4, R%NCOLS, R%NROWS, 1, R%XLLCORNER, R%YLLCORNER, R%CELLSIZE, 0., 'FLOAT     ')

   CALL MPI_REDUCE(EMBER_FLUX_ACCUMULATED%R4(:,:,1),  INTERMEDIATE_R4%R4(:,:,1), ANALYSIS_NCOLS*ANALYSIS_NROWS, MPI_REAL, MPI_SUM, &
