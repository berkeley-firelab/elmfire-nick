...
IF (USE_SDI                .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(27)) CALL READ_BSQ_RASTER_SLICE (SDI               , FUELS_AND_TOPOGRAPHY_DIRECTORY, SDI_FILENAME, 1, 1               )
! Majid_bav::added below
IF (ENABLE_EXTENDED_ATTACK .AND. EXTENDED_ATTACK_MODEL .EQ. 1 .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(26)) CALL READ_BSQ_RASTER_SLICE (PCL, FUELS_AND_TOPOGRAPHY_DIRECTORY, PCL_FILENAME, 1, 1)
...
REAL(8), INTENT(IN) :: TIMENOW !Majid_bav: modified this
...
IF (DUMP_SPREAD_RATE .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
   ...
   !Majid_bav::added below:: debug raster output
   I2DUMPME%I2(:,:,1) = NINT(I2DUMPME%NODATA_VALUE,2)
   C => LIST_BURNED%HEAD
   DO I = 1, LIST_BURNED%NUM_NODES
...
IF (USE_BLDG_SPREAD_MODEL .AND. BLDG_SPREAD_MODEL_TYPE .EQ. 2) THEN
   ...
   IF (DUMP_TOTAL_DFC_RECEIVED .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
      RDUMPME%R4(:,:,1) = TOTAL_DFC_WUI(:,:)
      FN = 'total_dfc_received_' // SEVEN // '_' // CTSEC
      CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IRANK_WORLD)
   ENDIF

   IF (DUMP_TOTAL_RAD_RECEIVED .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
      RDUMPME%R4(:,:,1) = TOTAL_RADIATION_WUI(:,:)
      FN = 'total_rad_received_' // SEVEN // '_' // CTSEC
      CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IRANK_WORLD)
   ENDIF

   IF (DUMP_FUEL_CONSUMPTION .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
      RDUMPME%R4(:,:,1) = FUEL_LOAD_REMAIN(:,:)
      FN = 'fuelload_transient_' // SEVEN // '_' // CTSEC
      CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IRANK_WORLD)
   ENDIF
   ...
   IF (DUMP_TRANSIENT_DFC) THEN
      ...
      FN = 'hf_dfc_transient_' // SEVEN // '_' // CTSEC
      CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IRANK_WORLD)
   ENDIF

   IF (DUMP_TRANSIENT_RAD) THEN
      RDUMPME%R4(:,:,1) = TRANSIENT_RADIATION_WUI(:,:)
      FN = 'hf_rad_transient_' // SEVEN // '_' // CTSEC
      CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IRANK_WORLD)
   ENDIF
...
! Are the following blocks necessary? EMBER_FLUX is a R4 raster everywhere else.
