...
def main() -> None:
    ...
    inputs_dir = case_dir / "data" / "inputs"
    outputs_dir = case_dir / "data" / "outputs"
    scratch_dir = case_dir / "data" / "scratch"
    for directory in (outputs_dir, scratch_dir):
        directory.mkdir(parents=True, exist_ok=True)

    # A zero adjustment factor forces zero normal velocity after the point ignition.
    write_inputs(
        inputs_dir, resolution=RESOLUTION, domain_size_m=DOMAIN_SIZE_M,
        adjustment=SPREAD_ADJUSTMENT, ignition_radius_m=None,
    )
    shutil.copy2(case_dir / "elmfire.data.in", inputs_dir / "elmfire.data")
    print(f"[OK] Generated no-spread inputs with adjustment={SPREAD_ADJUSTMENT:g}")
