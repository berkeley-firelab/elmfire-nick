...
def main() -> None:
    ...
    for resolution in RESOLUTIONS:
        output_dir = CASE_DIR / "data" / str(resolution) / "outputs"
    ...
    if missing or len(rows) != len(RESOLUTIONS):
        write_payload({
            "case_id": CASE_ID, "overall_status": "NOT EVALUABLE",
            "verification_passed": False, "required_outputs_complete": False,
            "missing_resolutions": missing, "metrics": [],
        })
        return

    dx = np.array([row["dx_m"] for row in rows])
    error = np.array([row["absolute_error_m"] for row in rows])
    order = observed_order(dx, error)
    finest = min(rows, key=lambda row: row["dx_m"])
    ...
    convergence_plot(dx, error, "Grid spacing (m)", FIG_DIR / "convergence.pdf")
    plot_domain(finest_phi, "CASE17 finest-grid final level-set field", FIG_DIR / "whole_domain_result.pdf")

    metrics = [
        {"name": "observed spatial order", "expected": MINIMUM_OBSERVED_ORDER,
         "calculated": order, "units": "-", "tolerance": ">= threshold",
         "status": "PASS" if order_pass else "FAIL"},
        {"name": "finest-grid radius error", "expected": 0.0,
