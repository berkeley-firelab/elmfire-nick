...
EXPECTED_NAMES = {
    "maximum HRR curve normalized L1 error",
    "double-peak HRR scaling error",
    "maximum DFC full-stack normalized L1 error",
    "maximum radiation full-stack normalized L1 error",
    "spatial nonburnable scaling error",
    "target-table absorptivity scaling error",
    "HRR_ELLIPSE_ADJ inverse-square error",
    "maximum radiation outside source cutoff",
    "maximum transient-to-total integration error",
    "two-source superposition error",
    "spatial nonburnable-raster response",
    "maximum arrival-mask mismatch count",
    "maximum unexpected HRR source count",
}
EXPECTED_VARIANTS = {
    "curve_base", "curve_double_peak", "curve_short", "curve_long_plateau",
    "target_nbf_0", "target_nbf_0p5", "target_nbf_1",
    "target_abs_0", "target_abs_0p4", "target_abs_1",
    "adj_0p25", "adj_0p5", "adj_0p75", "adj_1p0",
    "source_left", "source_right", "source_pair",
    "inert_rasters_low", "inert_rasters_high",
}
...
def load_payload() -> dict:
    ...
    base_valid = (
        payload.get("case_id") == "CASE44_WHP"
        ...
        and payload.get("required_variant_count") == 19
        and isinstance(payload.get("completed_variant_count"), int)
        and 0 <= payload["completed_variant_count"] <= 19
    ...
    complete_valid = not complete_decision or (
        ...
        and payload.get("completed_variant_count") == 19
    ...
    if not (base_valid and complete_valid and noncomplete_valid):
        return {
            "case_id": "CASE44_WHP", "overall_status": "NOT EVALUABLE",
            "workflow_status": "INCOMPLETE", "verification_passed": False,
            "required_outputs_complete": False, "required_variant_count": 19,
