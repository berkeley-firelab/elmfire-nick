#!/usr/bin/env python3
"""Prepare all variants in the merged structural-resolution WUI verification case.
...
def ignition_coordinates(source_mask, structure_ids, dx):
    ...
    ``new_phi`` initializes every represented source cell.  One namelist point
    per physical source structure is sufficient to seed the same footprints
    without approaching ELMFIRE's fixed 301-entry ignition-coordinate arrays.
    """
...
def write_variant(case_dir, template, case, spec, feature_available):
    ...
    # GeoTIFF rows are north-to-south.  The physical 2-D domain occupies the
    # rectangle immediately inside the two-cell halo on every side.
    ...
    present_x = structure_x_index[building_mask]
    first_structure_column = int(present_x.min())
    ...
    nominal_dt = float(spec["cfl"]) * dx / wind_m_s
    requested_tstop = float(spec["tstop_s"])
    step_count = int(math.ceil(requested_tstop / nominal_dt - 1.0e-12))
    nominal_product = step_count * nominal_dt
    if math.isclose(
        nominal_product, requested_tstop, rel_tol=0.0,
        abs_tol=max(1.0e-9, 1.0e-12 * requested_tstop)
    ):
        dt = nominal_dt
        aligned_tstop = nominal_product
    else:
        dt = requested_tstop / step_count
    ...
    return {
        **spec,
        ...
        "requested_cfl": float(spec["cfl"]),
        "actual_cfl": wind_m_s * dt / dx,
...
def main():
    """Read the merged contract and prepare all five reference variants."""
